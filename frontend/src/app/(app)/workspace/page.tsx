...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    const h = headers();
    fetch(`${env.NEXT_PUBLIC_API_URL}/v1/workspace/sessions`, { headers: h }).then(r => r.ok ? r.json() : null).then(d => {
      if (d) {
        const s = d.sessions || [];
        setSessions(s);
        if (s.length > 0) {
    ...
    fetch(`${env.NEXT_PUBLIC_API_URL}/v1/intelligence/recommendations`, { headers: h }).then(r => r.ok ? r.json() : null).then(d => {
      if (d) setRecommendations(d.recommendations || []);
    });
  }, [headers]);

  useEffect(() => {
    if (!activeSession) return;
    fetch(`${env.NEXT_PUBLIC_API_URL}/v1/workspace/session/${activeSession}`, { headers: headers() }).then(r => r.ok ? r.json() : null).then(d => {
    ...
    <div className="flex gap-4 h-[calc(100vh-6rem)]">
      ...
      <div className="flex-1 flex flex-col min-w-0 font-sans">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={createSession} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-300 border border-blue-400/20 bg-blue-500/[0.06] rounded-lg hover:bg-blue-500/[0.1] transition-colors">
        ...
        <div className="flex gap-2 mt-3">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.02] text-sm text-white placeholder-white/25 focus:outline-none focus:border-blue-400/30 transition-colors"
            disabled={sending}
          />
          <button
            onClick={sendMessage}
            disabled={sending || !input.trim()}
            className="px-4 py-2.5 bg-blue-500/20 border border-blue-400/20 text-blue-300 rounded-xl text-sm disabled:opacity-30 hover:bg-blue-500/30 transition-colors"
      ...
      {/* Action Center */}
      <div className="w-72 shrink-0 hidden lg:block overflow-y-auto font-sans">
        <GlassPanel>
