    ...
    <main className="container mx-auto px-4 py-12 animate-fade-in">
      ...
      {resumes.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 animate-fade-in">
          <DocumentTextIcon className="h-16 w-16 text-gray-700 mb-4" />
        ...
        <div className="grid gap-4">
          {resumes.map((resume, i) => (
            ...
            >
              ...
              <button className="px-4 py-1 rounded bg-purple-500 hover:bg-purple-600 text-white text-sm font-semibold transition-all">View</button>
            </div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {resumes.map((resume, i) => (
            <div
              key={resume.id}
              className="rounded-xl bg-gray-900/60 p-6 border border-gray-800 shadow-lg flex flex-col items-center gap-4 hover-lift hover:shadow-2xl transition-all"
              style={{ animationDelay: `${0.1 * i}s` }}
            >
